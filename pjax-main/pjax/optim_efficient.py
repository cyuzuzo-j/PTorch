...
def gather_and_average_outputs(computation: Computation, inputs: FrozenDict, graph: nx.DiGraph):
    """Gather input values from children and average them using recursive mean.

    Uses the formula: mean(N) = (mean(N-1) * (N-1) + x_N) / N
    to avoid allocating a stacked array of all child outputs.
    """
    ...
    n = 0
    mean = None
    for child in graph.successors(computation):
        for item in get_childs_input(child, computation, inputs, graph):
            n += 1
            if mean is None:
                mean = item
            else:
...
class Optimizer(ABC):
    ...
    def update(self, fun, params: FrozenDict, steps_per_update=None):
        ...
        # optimize using fori_loop with recursive running mean for the loss
        def loss_fn(old, new):
        ...
        if self.uses_velocity:
            def body_fn(i, carry):
                vars_, vel, mean_loss = carry
                new_vars, new_vel = self._step(vars_, *projections, vel)
                loss = loss_fn(vars_, new_vars)
                new_mean = (mean_loss * i + loss) / (i + 1)
                return (new_vars, new_vel, new_mean)

            init_carry = (inputs, velocity, jnp.float32(0.0))
            inputs, velocity, mean_loss = jax.lax.fori_loop(0, steps_per_update, body_fn, init_carry)
        else:
            def body_fn(i, carry):
                vars_, mean_loss = carry
                new_vars = self._step(vars_, *projections)
                loss = loss_fn(vars_, new_vars)
                new_mean = (mean_loss * i + loss) / (i + 1)
                return (new_vars, new_mean)

            init_carry = (inputs, jnp.float32(0.0))
            inputs, mean_loss = jax.lax.fori_loop(0, steps_per_update, body_fn, init_carry)
        ...
        return freeze(new_params), mean_loss
