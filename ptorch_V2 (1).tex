...
\author{%
  Jeff Cyuzuzo Jambe\thanks{cyuzuzo.vercel.app} \\
  KU Leuven\\
  \texttt{cyuzuzo.jambe@gmail.com} \\
...
\begin{equation} \label{eq:feasibility} \tag{P}
    \textnormal{find } \omega \in \left(\bigcap_{b=1}^B\bigcap_{i=1}^N A_{bi}\right) \bigcap \left(B_{\rm in} \cap B_{\rm out}\right) \bigcap \left(\bigcap_{i=1}^N C_i\right).
\end{equation}
...
\begin{enumerate}[leftmargin=2em]
    ...
    \item \textbf{B}atch (data) constraints: The inputs and outputs of the network are fixed to match the current training batch, i.e., $\omega \in B_{\rm in} \cap B_{\rm out}$ means that $h_b^{(0)} = x_b$ and $h_b^{(N)} = y_b$ for each $b=1,\ldots,B$.
...
As discussed previously, in \cite{elser2019learning,bergmeister2025projection}, the duplication of the network parameters per batch is a primary memory bottleneck. A naive implementation requires constructing an $N_{\mathrm{in}} \times N_{\mathrm{out}} \times B$ tensor, where $N_{\mathrm{in}}, N_{\rm out}$ are the input and output dimensions of the linear layer respectively, and $B$ is the batch size. This $\mathcal{O}(N_{\mathrm{in}}N_{\mathrm{out}}B)$ memory footprint routinely causes out-of-memory errors on standard hardware, that we also empirically observe in \cref{sec:exp-memory-optimizer}. The authors of PJAX \cite{bergmeister2025projection} note: 
...
\[
     \minimize_{\overline{h}^{i+1}_b\in\mathbb{R};\,\overline{h}^i_b,\overline{\theta}_b^i[j,:]\in\mathbb{R}^{N_{\rm in}}}\; \ \lVert \overline{h}^i_b-h^i_b\rVert^2 + \alpha^2\lVert \overline{\theta}_b^i[j,:]-{\theta}_b^i[j,:]\rVert^2 + \gamma^2\,(\overline{h}^{i+1}_b-h^{i+1}_b)^2 \text{ s.t. } \ \overline{h}^{i+1}_b = \overline{\theta}_b^i[j,:]^\top \overline{h}^i_b
\]
...
\[
    \bar{\Theta} = \frac{1}{B} \left( \Theta \odot (U\mathbf{1}_{B\times N_{\rm in}}) + V H \right),
\]
where $\odot$ denotes the Hadamard product and $\mathbf{1}_{B\times N_{\rm in}}$ is a $B\times N_{\rm in}$ matrix of all ones. The second term is evaluated as a single $N_{\mathrm{out}} \times B$ by $B \times N_{\mathrm{in}}$ matrix multiplication. Because these computations are fused, no three-dimensional intermediate tensor is ever formed. As a result, peak memory usage drops from $O(N_{\mathrm{in}}N_{\mathrm{out}}B)$ to $\mathcal{O}(N_{\mathrm{in}}B + N_{\mathrm{out}}B + N_{\mathrm{in}}N_{\mathrm{out}})$.
...
\min_{\bar{h} \in \mathbb{R}^{N_{\rm in}}, \bar{\theta} \in \mathbb{R}^{N_{\rm out} \times N_{\rm in}}, \bar{h}^+ \in \mathbb{R}^{N_{\rm out}}} \| (\bar{h}, \bar{\theta}, \gamma^2 \bar{h}^+) - (h, \theta, \gamma^2 h^+) \|_\infty \quad \text{s.t.} \quad \bar{\theta} \bar{h} = \bar{h}^+
\end{equation}
...
The choice of projection norm fundamentally alters training dynamics. The $\ell_\infty$ projection is computationally more expensive but converges faster per optimization step and reaches a higher final accuracy. As the batch size grows, the $\ell_2$ implementation becomes substantially faster per wall-clock time than the $\ell_\infty$ projection. Consequently, with larger batch sizes, it takes longer in wall-clock time for the $\ell_\infty$ curve to overtake the $\ell_2$ baseline.
\begin{figure}[ht]
    \centering
...
\end{figure}
In the current framework, the number of backward passes per data batch is fixed at 1, as this allows for a substantially more efficient and elegant implementation. However, this choice may not be optimal, since using multiple backward passes can improve performance per step
\begin{figure}[ht]
\centering
...
One hidden-state optimization strategy implemented in $\mathcal{P}$Torch applies Muon\cite{jordan2024muon} specifically, the Polar Express operationalization\cite{amsel2025polar} \emph{without} the rescaling step to the hidden-state residuals. This stabilizes the target signal as depth increases. Although this treatment causes a small accuracy regression on shallow networks, it benefits deeper networks far more than it harms shallow ones. 
\begin{figure}[ht]
\centering
...
\section{Breaking the Depth Barrier: From Eligibility Traces to Direction Pipes}
\label{sec:td-trace}

This appendix documents, end to end, an investigation into the vanishing-target
problem of \cref{sec:vanishing-target}: a first attempt that imports eligibility
traces from temporal-difference (TD($\lambda$)) reinforcement learning, a sequence of
measurements explaining precisely \emph{why} that attempt restores signal magnitude
but not performance, and the mechanism those measurements force --- a
\emph{direction pipe} --- which closes the depth gap to gradient-based training on
the benchmarks of \cref{sec:exp-vanishing}. All quantities reported here were
measured with the released code; the relevant switches are listed in
\cref{sec:td-repro}. We do not shorten the account: the negative results are as
load-bearing as the final method, because they localize the failure of
projection-based learning at depth to a single, measurable quantity.

\vspace{0.5em}
\begin{tcolorbox}[shift_card, title=Summary]
The vanishing-target theorem of \cref{sec:vanishing-target} is a statement about
\emph{amplitude}, but the binding constraint at depth turns out to be
\emph{direction}. The backward sweep already transports the correct (adjoint)
direction; it merely attenuates it geometrically until numerical annihilation.
Re-amplifying that direction locally --- with a scalar, frame-invariant floor,
capped into the solver's linear-transport window, and with a known solver artifact
deflated --- lifts a 4-layer ReLU MLP on MNIST from $92.1\%$ to $97.2\%$ test
accuracy, at parity with the Adam baseline ($97.1\%$), while leaving the
$\lambda_{\rm TD}=0$ framework bitwise unchanged.
\end{tcolorbox}
\vspace{0.5em}

\subsection{Setup and notation}
\label{sec:td-notation}
Throughout, we use the chain-network notation of \cref{sec:math_desc} and write the
\emph{residual} delivered to hidden state $i$ as
\[
    r^{(i)} \;:=\; h^{(i)} - \bar h^{(i)}, \qquad \delta^{i} = \lVert r^{(i)}\rVert_2 ,
\]
and the \emph{seed} residual produced by the proximal loss step
\eqref{eq:proximal-step} as $s := r^{(N)} = h^{(N)} - \operatorname{prox}_{\lambda
\mathcal{L}}(h^{(N)}, y)$. The trace parameter introduced below is written
$\lambda_{\rm TD}$ and is entirely distinct from the proximal strength $\lambda$ of
\cref{sec:moreau}.

Two implementation facts from \cref{app:walk-through} matter for everything that
follows. First, the quantity that flows backward through \texttt{autograd} is the
\emph{target} $\bar h^{(i)}$, not the residual; residuals are only formed once, when
the projection optimizers convert \texttt{p.grad} $\gets$ \texttt{p} $-$
\texttt{p.grad} (\cref{sec:relaxation}). Second, no state object persists across the
node-level backward calls, so any quantity that must travel \emph{along} the sweep
(as opposed to \emph{through} it) has to live in module-level state, seeded at the
loss node --- which \texttt{autograd} is guaranteed to execute first --- and updated
at each bilinear solve in the engine's reverse-topological order. This construction
is sound for chain graphs, which is the setting of this appendix.

\subsection{A first attempt: an eligibility trace over depth}
\label{sec:td-variant-a}
The backward sweep of \cref{alg:minibatch} bootstraps each layer's target off the
next layer's already-updated target. Structurally this is one-step bootstrapping:
mapping \emph{depth to time}, the projection residual plays the role of the TD
error, the magnitude $\lVert\Delta\theta^{(i+1)}\rVert^2 +
\lVert\Delta h^{(i+1)}\rVert^2$ consumed by each layer (the decrement in the
vanishing-target bound of \cref{sec:proof-vanishing-targets}) plays the role of the
reward consumed at each step, and the geometric decay of $\delta^i$ is the analogue
of TD(0)'s slow long-horizon credit assignment. In reinforcement learning this is
repaired by eligibility traces; imported here, the natural update is a
\emph{backward-view trace over depth}: seed a running vector $g \gets s$ at the loss
node, and at each bilinear solve blend it with the local residual and deliver the
blend instead,
\begin{equation}
    g \;\gets\; \lambda_{\rm TD}\, g \;+\; (1-\lambda_{\rm TD})\, r^{(i)},
    \qquad \bar h^{(i)}_{\rm delivered} \;=\; h^{(i)} - g .
    \label{eq:td-vector}
\end{equation}
At $\lambda_{\rm TD}=0$ this reduces \emph{exactly} to the current sweep; at
$\lambda_{\rm TD}=1$ the undecayed output residual reaches every layer --- the
Monte-Carlo / skip-connection limit. Because the delivered target becomes the next
solve's output-side constraint, the blend enters the recursion, i.e.\
\eqref{eq:td-vector} is the recursive $\lambda$-return; the alternative (blending
``on the side'' while the pure chain propagates) is provably a no-op in this
architecture, because a hidden-state target has no effect \emph{other} than serving
as the next constraint. Weight targets are never blended.

On the identity-recovery testbed of \cref{sec:exp-vanishing}, this trace does
exactly what the analogy promises for \emph{magnitude}: the step-1 profile
$\delta^n_k$ decays at rate $\approx\max(\lambda_{\rm TD}, c)$ per layer, where
$c\approx0.06$ is the intrinsic contraction measured below, and is perfectly flat at
$\lambda_{\rm TD}=1$. It also does nothing --- or worse --- for \emph{performance}:
final MSE is unchanged at depth $8$, modestly better at depth $4$, and $16\times$
worse at depth $2$, with a $\lambda_{\rm TD}$-independent noise floor. On the
MNIST MLPs it is likewise not the magnitude restoration that matters
(\cref{tab:td-mnist}). The reason is the subject of the next subsection.

\vspace{0.5em}
\begin{tcolorbox}[shift_card]
The vector trace restores \emph{magnitude without information}: at hidden state $i$
the delivered vector has measured alignment
$\cos\!\bigl(g,\, \nabla_{h^{(i)}}\mathcal{L}\bigr) \approx 0.05\text{--}0.24$,
i.e.\ at the level $1/\sqrt{d}\approx0.18$ of a \emph{random} vector in $d=32$
dimensions. Only $\cos^2 \approx 1/d \approx 3\%$ of the injected energy is signal.
\end{tcolorbox}
\vspace{0.5em}

\subsection{Anatomy of the failure: where direction is created, transported, and destroyed}
\label{sec:td-anatomy}

\subsubsection{The bilinear solve is adjoint transport with a lossy amplitude budget}
\label{sec:td-adjoint}
Linearize the fused bilinear projection of \cref{subsec:fused} around the current
point: for the constraint $(\!A+\delta\!A)(B+\delta\!B) = AB - s$ under the cost
$\lVert\delta\!A\rVert^2 + \alpha\lVert\delta\!B\rVert^2$, the minimum-norm solution
distributes the residual as
\[
    \delta\!A \;\propto\; s\,B^\top M, \qquad
    \delta\!B \;\propto\; \alpha^{-1} A^\top s\, M ,
\]
for a positive-definite weighting $M$: \textbf{the activation share of the residual
is transported through $B^\top$ --- the adjoint, i.e.\ exactly backpropagation's
transport map} --- emerging from the projection geometry itself, with no chain rule
invoked. This is visible in the closed-form reconstruction of
\cref{sec:proj-mechanics} (the $t/(\alpha - t^2)\cdot B^\top$ term). Measured at
$\alpha=1$ on the identity-task geometry (Kaiming init, $d=32$, batch $16$):
$\cos(\delta\!A,\, sB^\top) = 0.95$.

What decays with depth is only the \emph{amplitude}. The cost split assigns the
activations the fraction
\begin{equation}
    \tau \;\approx\; \frac{\alpha\, q_B}{\alpha\, q_B + q_A},
    \qquad
    q_A = \text{mean row } \lVert A\rVert^2 \approx 33, \quad
    q_B = \text{mean col } \lVert B\rVert^2 \approx 2.1
    \label{eq:td-tau}
\end{equation}
at initialization, giving $\tau \approx 0.06$ per layer --- matching the
order-of-magnitude-per-layer collapse of \cref{fig:depth-barrier} and identifying
the ``constant'' $c$ of the vanishing-target phenomenon as a \emph{cost-ratio
artifact}, not a law of nature: activations are simply $\sim\!16\times$ heavier than
weights in the projection metric, so the solve satisfies the constraint by moving
weights. The transported direction survives this attenuation until the amplitude
reaches solver / dtype noise ($\sim\!10^{-7}$), at which point it is annihilated:
at the sixth hidden state from the output we measure $\lVert r\rVert = 2\cdot10^{-8}$
with alignment $0.07$.

\vspace{0.5em}
\begin{tcolorbox}[shift_card]
Restatement of the depth barrier: \emph{correctly-directed amplitude decays
geometrically until numerical annihilation}. Until the underflow, the information is
intact; the problem is numerics, not credit routing.
\end{tcolorbox}
\vspace{0.5em}

\subsubsection{Identity transport of vectors across layers is information-free}
The correct first-order target direction at depth $k$ is $J_k^\top s$ with $J_k$ the
downstream Jacobian (for the linear testbed, the product of the downstream weight
matrices). The trace \eqref{eq:td-vector} delivers $s$ \emph{verbatim} --- identity
transport --- which is valid only when $J_k \approx I$, i.e.\ in residual
architectures. For random $J_k$,
$\mathbb{E}\lvert\cos(s, J_k^\top s)\rvert \approx 1/\sqrt{d} = 0.177$ at $d=32$,
matching the measured $0.05$--$0.24$. Transporting a \emph{vector} across a layer
requires the frame map $\theta^\top$ --- which is literally backpropagation and
forfeits the gradient-free premise. Scalars, by contrast, are frame-invariant; this
observation drives the design in \cref{sec:td-pipe}.

\subsubsection{Repetition cannot rescue a misaligned signal}
A natural hope is that the $K$ inner sweeps of \cref{alg:minibatch} could compound a
weak signal. They cannot: per sweep, the useful component reaching depth $k$ is
$c^{\,k}$ (plain sweep) or a fixed $\sim\!1/\sqrt{d}$ fraction (vector trace), so $K$
sweeps accumulate at most \emph{linearly}, $K\,c^{\,k}$, against an
\emph{exponential} deficit --- rescue requires $K \approx c^{-k} \approx 10^{k}$.
Worse, for the trace the misaligned component originates from the same seed and
therefore accumulates \emph{coherently}, at the same rate as the signal: the
signal-to-noise ratio is $\sim\!1/\sqrt d$ independently of $K$. Repetition
multiplies magnitude, never information.

\subsubsection{Amplification without care destroys the transported direction}
\label{sec:td-saturation}
Equation \eqref{eq:td-tau} suggests an innocent fix: raise $\alpha$ until
$\tau\to1$. Measured against the exact solver, this fails in an instructive way
(\cref{tab:td-alpha}): amplitude indeed arrives, but the alignment of the
activation move with $sB^\top$ collapses from $0.95$ to $0.1$--$0.28$ once $\alpha$
passes $q_A/q_B$. The mechanism is the solver's own reconstruction: as the Newton
parameter $t$ grows, the activation update is dominated by the
$t^2/(\alpha-t^2)\cdot A$ term --- a rescaling of the activation \emph{itself},
uncorrelated with the transported signal. The same failure appears if one rescales
the residual back up to seed scale naively: the delivered vector becomes
$91$--$99.6\%$ row-parallel to the activation at depth. Direction is transported
faithfully only while the residual is \emph{small relative to the activations}:

\begin{table}[ht]
\centering
\caption{Measured transport fidelity of the fused $\ell_2$ bilinear solve.
Left: alignment $\cos(\delta\!A,\, sB^\top)$ vs.\ the cost ratio $\alpha$ (unit
residual). Right: alignment vs.\ the relative residual amplitude
$\lVert s\rVert/\lVert Z\rVert$ at $\alpha=1$ --- the \emph{fidelity window}.}
\label{tab:td-alpha}
\small
\begin{tabular}{lcc}
\toprule
$\alpha$ & $\tau$ (pred.) & $\cos(\delta\!A, sB^\top)$ \\
\midrule
$1$              & $0.06$ & $\mathbf{0.95}$ \\
$q_A/q_B$        & $0.50$ & $0.11$ \\
$9\,q_A/q_B$     & $0.90$ & $0.24$ \\
$100\,q_A/q_B$   & $0.99$ & $0.28$ \\
\bottomrule
\end{tabular}
\hspace{1.5em}
\begin{tabular}{lc}
\toprule
$\lVert s\rVert/\lVert Z\rVert$ & $\cos$ \\
\midrule
$10^{-6}$ & $0.24$ \\
$10^{-4}$--$10^{-1}$ & $0.94$--$0.95$ \\
$3\cdot10^{-1}$ & $0.91$ \\
...
The window is wide --- four orders of magnitude --- bounded below by numerical noise
and above by $t$-saturation. Two practical corollaries. First, the warm-started
Newton cache is a hazard: a stale $t$ from a \emph{saturated} solve pins the next
one-step solve ($\cos = 0.31$, $\lVert\delta\!A\rVert$ frozen at the stale scale
regardless of the actual residual); diagnostic passes must snapshot and restore the
per-layer caches. Second, and unexpectedly, \emph{more} Newton iterations per solve
are harmful in this regime (\cref{tab:td-mnist}): the single clamped step of the
released solver acts as essential damping, and solving the local projection more
exactly produces more aggressive moves whose artifact component grows --- a
projection-based analogue of the observation that exact line search can hurt
stochastic optimization.

\subsection{The direction pipe}
\label{sec:td-pipe}
The measurements above leave exactly one design open, and it is a computer-science
fix rather than a mathematical one. Under the nonlinear relaxation of
\cref{sec:relaxation} with Muon \cite{jordan2024muon}, weight-update
\emph{magnitude} is free --- the polar factor is scale-invariant --- so the backward
chain does not need to deliver magnitude at all. It needs to deliver
\emph{direction}, at \emph{any} amplitude inside the fidelity window of
\cref{tab:td-alpha}. The chain therefore becomes a \emph{direction pipe}:

\begin{algorithm}[htbp]
\caption{Direction-pipe backward sweep (norm-mode trace; replaces lines 3--5 of
\cref{alg:minibatch}). $\lambda_{\rm TD}=0$ recovers \cref{alg:minibatch} exactly.}
\label{alg:td-pipe}
\begin{algorithmic}[1]
\State $\phi \gets \lVert s\rVert_2$ \Comment{scalar floor, seeded at the loss residual}
\For{$i=N-1$ \textbf{downto} $0$} \Comment{reverse-topological order}
    \State $(\bar h^{(i)}, \bar\theta^{(i+1)}, \cdot) \in \Pi_{A_{i+1}}(h^{(i)}, \theta^{(i+1)}, h^{(i+1)})$; \quad $r \gets h^{(i)} - \bar h^{(i)}$
    \If{layer $i{+}1$ is bilinear} \Comment{trace update points}
        \State $r \gets r - \operatorname{rowproj}_{h^{(i)}}(r)$ \Comment{\emph{deflation}: strip the $A$-parallel artifact}
        \State $\phi \gets \lambda_{\rm TD}\,\phi + (1-\lambda_{\rm TD})\,\lVert r\rVert_2$ \Comment{scalar trace over depth}
    \EndIf
    \State $a \gets \min\bigl(\phi,\; \varepsilon_{\rm lin}\,\lVert h^{(i)}\rVert_2\bigr)$ \Comment{cap into the fidelity window}
    \State deliver $\bar h^{(i)} \gets h^{(i)} - \max\!\bigl(1,\, a / \lVert r\rVert_2\bigr)\, r$ \Comment{amplify; never shrink}
    \State $\theta^{(i+1)} \gets \operatorname{Optimizer}(\theta^{(i+1)} - \bar\theta^{(i+1)})$ \Comment{weights stay purely local}
\EndFor
\end{algorithmic}
\end{algorithm}

Each ingredient answers one measured failure:
\begin{enumerate}[leftmargin=2em]
    \item \textbf{Scalar floor $\phi$ (frame invariance).} Only the \emph{norm}
    crosses layers, so nothing is mis-transported (\cref{sec:td-anatomy}); the
    mechanism is width-agnostic for free, requiring no dimension bridge between
    layers of different sizes. At $\lambda_{\rm TD}=0$, $\phi = \lVert r\rVert$ and
    the rescale is exactly $1$: the entire mechanism is a strict no-op, verified
    bitwise against the pre-trace framework.
    \item \textbf{Seed coupling (annealing).} $\phi \propto \lVert s\rVert$
    vanishes as the loss does. This is the decisive difference with the
    orthogonalization-without-rescaling heuristic of
    \cref{sec:exp-relaxation-location} (\cref{fig:orthogonalization}), whose
    $\lVert A\rVert$-coupled step size never anneals and therefore keeps injecting
    fixed-magnitude perturbations at convergence --- the reason its ``fixed''
    variants regress on shallow networks. Empirically the direction pipe
    \emph{supersedes} that heuristic (\cref{tab:td-mnist}).
    \item \textbf{Linearity cap $\varepsilon_{\rm lin}$.} The restored amplitude is
    capped at $\varepsilon_{\rm lin}\lVert h^{(i)}\rVert$ (default $10^{-2}$),
    keeping \emph{every} hop inside the fidelity window of \cref{tab:td-alpha}.
    Without it, the floor pushes each subsequent solve into $t$-saturation and
    alignment collapses within two hops (measured: $0.83 \to 0.52 \to 0.15 \to
    {-0.1}$); with it, alignment at depth $8$ is $+0.78, +0.42, +0.26, +0.25, +0.20,
    +0.18, \mathbf{+0.17}$ from the output down to the first hidden state ---
    against exactly $0$ for the plain sweep and $\pm 1/\sqrt d$ noise for the vector
    trace.
    \item \textbf{Repeaters at non-bilinear nodes.} Activation, pooling and
    normalization projections consume amplitude between the bilinear solves (the
    $\ell_2$ ReLU projection halves the active-branch residual). Each such node
    applies the same capped rescale \emph{without updating $\phi$}: the
    $\lambda_{\rm TD}$-blend happens once per layer, and the non-bilinear nodes
    become amplitude-transparent. On the 4-layer MNIST MLP this is worth
    $+0.4$ points; on the 1-layer MLP (a single ReLU) it is a no-op, as the
    mechanism predicts.
    \item \textbf{Deflation.} The per-row component of $r$ parallel to the
    activation is the solver's self-rescaling artifact
    (\cref{sec:td-saturation}) --- $20$--$25\%$ of residual energy even inside the
    window --- and carries no information about the constraint violation. Removing
    it before the rescale is the single largest gain in \cref{tab:td-mnist}
    ($+2.7$ points), more than every other ingredient combined.
\end{enumerate}
...
On the testbed of \cref{sec:exp-vanishing}, with the floor (before deflation was
introduced), $\lambda_{\rm TD}=0.3$ improves final MSE by $75\times$ at depth $4$
($8.6\cdot10^{-5}$ vs.\ $6.4\cdot10^{-3}$) and reaches the all-layers Adam
reference at depth $8$ ($1.17\cdot10^{-2}$ vs.\ $1.14\cdot10^{-2}$; plain sweep:
$2.22\cdot10^{-2}$), with non-overlapping seed ranges. Moderate
$\lambda_{\rm TD}\in[0.3,0.5]$ is optimal; at $\lambda_{\rm TD}\geq 0.9$ the floor
stops annealing relative to the local scale and final loss degrades below the
baseline --- the predicted cost of abandoning the pure-projection regime, appearing
as a chronic noise floor rather than divergence.

Two methodological points made this measurable. First, the na\"ive expectation that
``a single linear layer can invert the frozen rest of the stack'' --- which would
render final loss insensitive to propagation --- is false in practice: products of
$k$ random Kaiming matrices have condition number $\sim\!10^{2}, 10^{5}, 10^{8.7}$
at $k=1,3,7$, so the required inverse is unreachable under bounded-norm updates;
\emph{top-layer-only} training with true gradients stalls at MSE $0.96$ at depth
$8$ where all-layers training reaches $0.30$. Early layers are genuinely required
(for a well-conditioned factorization, not for expressivity), and the
final-loss-vs-depth curve of \cref{fig:depth-barrier} \emph{is} therefore a
propagation metric. Second, the honest reference for that curve is an all-layers
gradient baseline at equal budget --- itself far from zero at depth ($1.1\cdot
10^{-2}$ at $L=8$): depth is hard for everyone on this task, and the meaningful
claim is parity, not perfection.

\paragraph{MNIST MLPs.}
\cref{tab:td-mnist} reports the classification benchmark of
\cref{sec:exp-vanishing}(b) (ReLU MLPs $784\to512^{k}\to10$, proximal cross-entropy,
\texttt{ProjectionMuonV2}, batch $32$, $5{,}000$ steps, three seeds; the trace is
seeded at the cross-entropy proximal node).

\begin{table}[ht]
\centering
\caption{Test accuracy (\%, mean of 3 runs) on MNIST. ``Muon rescue'' is the
hidden-state orthogonalization of \cref{fig:orthogonalization}; ``pipe'' is
\cref{alg:td-pipe} with $\lambda_{\rm TD}=0.3$, $\varepsilon_{\rm lin}=10^{-2}$.
The full recipe is the last $\mathcal{P}$Torch row.}
\label{tab:td-mnist}
\small
\begin{tabular}{lcc}
\toprule
Configuration & 1-layer & 4-layer \\
\midrule
plain sweep ($\lambda_{\rm TD}=0$)             & 95.88 & 92.14 \\
Muon rescue (no trace)                          & 95.80 & 93.63 \\
Muon rescue $+$ pipe                            & 95.77 & 93.81 \\
pipe, bilinear solves only                      & 97.44 & 94.11 \\
pipe $+$ repeaters                              & 97.45 & 94.48 \\
\textbf{pipe $+$ repeaters $+$ deflation}       & \textbf{97.60} & \textbf{97.21} \\
\midrule
pipe $+$ repeaters, 3 Newton steps per solve    & --- & 84.73 \\
plain sweep, 3 Newton steps per solve           & --- & 86.51 \\
\midrule
\texttt{torch} Adam reference                   & 97.76 & 97.08 \\
\bottomrule
\end{tabular}
\end{table}
...
\vspace{0.5em}
\begin{tcolorbox}[shift_card]
With the full direction pipe, the projection-trained 4-layer MLP reaches parity
with its gradient-trained counterpart (97.21 vs.\ 97.08), and the framework's
depth tax (97.60 $\to$ 97.21) is \emph{smaller} than backpropagation's own
(97.76 $\to$ 97.08). The pipe also strictly dominates the earlier Muon-rescue
heuristic, and composing the two adds nothing --- they pull the same lever, but
only the pipe's floor anneals with the loss.
\end{tcolorbox}
\vspace{0.5em}

Additional null results worth recording: final accuracy is flat across
$\lambda_{\rm TD}\in[0.3,0.7]$; linearly annealing $\lambda_{\rm TD}\to0$ over
training changes nothing (late-training trace noise is not binding at this budget);
widening the cap to $3\cdot10^{-2}$ slightly hurts, consistent with
\cref{tab:td-alpha}. The gap that remained before deflation was therefore not a
tuning problem, and after deflation there is no gap left to tune.

\subsection{Scope, forfeited guarantees, and failure modes}
\label{sec:td-scope}
The monotone bound of \cref{sec:proof-vanishing-targets} relies on every delivered
target being a pure projection under the locally non-expansive $\Pi$. A rescaled
target corresponds to no feasible point, so the bound no longer constrains the
dynamics and $\delta$ \emph{can} grow --- this is the deliberate trade, and it is
observable: at $\lambda_{\rm TD}=1$ the \emph{local} residual grows by an order of
magnitude per layer toward the input at step 1, exactly the theorem failing in
reverse. In practice the regime $\lambda_{\rm TD}\leq0.7$ with the default cap is
mild (a bounded noise floor, annealing with the loss), while two combinations are
actively destructive and should be avoided: raising the per-solve Newton iteration
count (see \cref{tab:td-mnist}), and combining it with deflation (divergence: the
deflated residual shrinks, the floor amplifies harder, and the sweep exits the
fidelity window). Current limitations: the trace state assumes a chain graph
executed by one backward pass at a time; fan-out (\texttt{Branch}) nodes and the
$\ell_\infty$ bilinear solve do not yet participate; and the mechanism is validated
here on MLPs --- convolutional and attention architectures inherit the repeaters
untested. Residual architectures are a promising direction: skip connections make
identity transport valid ($J\approx I$), which is precisely the regime where even
the vector trace of \cref{sec:td-variant-a} becomes sound.

\subsection{Reproducibility}
\label{sec:td-repro}
\begin{tcolorbox}[reprocard, title=Reproducibility]
All mechanisms are gated behind configuration flags in
\texttt{ptorch.config}; the default configuration
(\texttt{td\_lambda}$\,=0$) is bitwise-identical to the framework without this
appendix (verified by \texttt{experiments/deep/test\_td\_lambda\_equiv.py}, which
reconstructs the pre-trace code and compares SHA-256 hashes of all weights after
training). The recommended recipe is \texttt{td\_mode="norm"},
\texttt{td\_lambda=0.3}, \texttt{td\_eps\_lin=1e-2}, \texttt{td\_deflate=True},
\texttt{use\_muon\_activations=False}, \texttt{num\_iters=1}. The vector trace of
\cref{sec:td-variant-a} is retained as \texttt{td\_mode="vector"} (negative
control). Identity-recovery sweeps: \texttt{experiments/deep/td\_lambda\_identity.py}
(logs per-layer $\lVert g\rVert$, local $\delta^{(i)}$, and the alignment
$\cos(r^{(i)}, \nabla_{h^{(i)}}\mathcal{L})$ against a true-gradient pass); MNIST
benchmarks: \texttt{experiments/mlp/bench\_ptorch.py --config deep\_mlp\_config.yaml}
with \texttt{--td-lambda}, \texttt{--td-deflate}, \texttt{--muon},
\texttt{--num-iters}. The measurement log behind every number in this appendix is
\texttt{experiments/deep/FINDINGS\_td.md}.
\end{tcolorbox}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.6\textwidth}
        \centering
        \include{drawings/location_extrema_inf_norm}
        \vspace{-1.0pt}
