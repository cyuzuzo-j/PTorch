...
hints.TRITON_MAX_BLOCK['Y'] = 65536
hints.TRITON_MAX_BLOCK['Z'] = 65536
...
class Hyperparameters:   
    ...
    num_layers = int(os.environ.get("NUM_LAYERS", 0))
    ...
    embed_lr = float(os.environ.get("EMBED_LR", 0.06))
...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        ...
        # Ensure ptorch CausalSelfAttention proj is zero-initialized as required by the architecture
        ...
        if self.tie_embeddings:
            if config.use_projections:
                logits_proj = MatMulProjection.apply(
                    x,
                    self.tok_emb.weight.T.contiguous(),
                    5, 1.0, 1.0, 1.0, None, False, False
                )
            else:
...
def main() -> None:
    ...
    if master_process and args.use_wandb:
        wandb_config = {k: getattr(args, k) for k in dir(args) if not k.startswith('_') and not callable(getattr(args, k))}
        wandb_config["use_projections"] = config.use_projections
    ...
    compiled_model =torch.compile( base_model)# torch.compile removed to save memory
    ...
    """
    optimizer_tok = ProjectionMuon([{"params":base_model.tok_emb.weight, "base_lr": args.scalar_lr}], lr=token_lr)
    """
    optimizer_scalar = ProjectionSGD(
    ...
    if base_model.lm_head is not None:
        ...
        """
        optimizer_head = ProjectionMuon([{"params": base_model.lm_head.weight, "base_lr": args.scalar_lr}], lr=args.scalar_lr, momentum=args.muon_momentum, ns_steps=args.muon_backend_steps)
    ...
    log0(f"use_projections:{config.use_projections}")
    ...
    while True:
        if step % 1000 == 0 and step > 0:
            config.update("use_projections", not config.use_projections)
            log0(f"step:{step} toggled use_projections to {config.use_projections}")
        ...
        if master_process and args.use_wandb:
            wandb.log({
                ...
                "train/approx_time_ms": approx_training_time_ms,
                "train/use_projections": int(config.use_projections),
            }, step=step)
