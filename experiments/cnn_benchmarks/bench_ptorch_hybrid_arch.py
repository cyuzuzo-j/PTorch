...
class SimpleCNN_PTorch(tnn.Module):
    ...
    def forward(self, x):
        ...
        x = x.flatten(1)
        # Bound the head's input so the projection target A_proj sits at a
        # comparable scale to x; the displacement (x - A_proj) flowing back
        # through Conversion then has a bounded, optimizer-meaningful magnitude.
...
FRAMEWORK = "ptorch_cnn_hybrid_arch"
...
def run(cfg, task_cfg, batch_size, run_number, device,
    ...
    # ── Separate parameters for hybrid optimization ───────────────────────────
    cnn_params = []
    head_params = []
    for name, param in model.named_parameters():
        # Any parameter belonging to the ptorch modules goes to head_params
        if "head" in name or "conversion" in name:
            head_params.append(param)
        else:
            cnn_params.append(param)

    # Plain SGD lr=1 on the backbone: the Conversion layer emits (x - A_proj)
    # as the "gradient", so one SGD step with lr=1 corresponds to one
    # target-propagation step. Adam / momentum / other lrs break this mapping.
    torch_optimizer = torch.optim.Adam(cnn_params)

    # 2. Custom PTorch Optimizer for the head
    ...
    ptorch_optimizer = OPTIM_MODULES[opt_name](head_params, **opt_kwargs)
    ...
    with tqdm.tqdm(total=cfg["max_steps"], unit="step",
        ...
        while step < cfg["max_steps"]:
            ...
            ptorch_optimizer.zero_grad()
            
            loss.sum().backward()
            
            # Step both optimizers
            torch_optimizer.step()
            ptorch_optimizer.step()
