...
def plot_task(df: pd.DataFrame, task_name: str, output_dir: str):
    ...
    ax.set_title(f"{dataset_label} CNN — Accuracy vs. Step")
    ...
    ax.set_ylabel("Validation Accuracy")
    ax.set_title(f"{dataset_label} CNN — Accuracy vs. Time")
    ax.legend(loc="lower right", fontsize=12, frameon=False)
...
def main():
    ...
    output_dir  = os.path.join(os.path.dirname(__file__), cfg.get("figures_dir", "../../images/cnn"))
