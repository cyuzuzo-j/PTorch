##################################################
###   Benchmark — pjax_orr (original)         ###
###   Alternating projections via optim       ###
##################################################
import sys, os
...
class MLP(nn.Module):
    ...
    def __call__(self, x):
        x = pjax_orr.reshape(x, (x.shape[0], -1))
...
def run(cfg, task_cfg, batch_size, run_number, key):
    ...
    opt_name   = cfg["pjax_orr_optimizer"]
    ...
    def step_fn(params, x, y):
        def apply_fn(p):
            ...
            return pjax_orr.cross_entropy(pred, y_oh)
        loss = jnp.mean(apply_fn(params))
        new_params, _ = optimizer.update(apply_fn, params)
