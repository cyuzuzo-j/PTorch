...
from ptorch.nn.modules import Linear as PLinear, ReLU as PReLU, LeakyReLU
...
from ptorch.core.ops import process_activation_target, CrossEntropyProjection
...
class MLP(tnn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(0, len(self.hidden_layers), 2):
            x_og = x.detach().requires_grad_(True)
            x = self.hidden_layers[i](x)
            x = self.hidden_layers[i + 1](x)
            if i>0:
                x = ResidualAdd.apply(x_og, x)
...
def run(cfg, task_cfg, batch_size, run_number, device):
    ...
    model = MLP([512,512,512,512], 784,classes=task_cfg["classes"]).to(device)
    ...
    with tqdm.tqdm(total=cfg["max_steps"], unit="step") as pbar:
        while step < cfg["max_steps"]:
            ...
            projected = CrossEntropyProjection.apply(output, y_oh)
