"""Diagnostic 11: Compare projections vs standard backprop, track weight norms."""
import sys, os
...
configs = [
    ("Projections ON + AP", True, "AlternatingProjections", {"lr": 1.0}),
    ("Projections OFF + Adam", False, "ProjectionAdam", {"lr": 0.001}),
    ("Projections OFF + SGD", False, "ProjectionSGD", {"lr": 0.01}),
]

for name, use_proj, opt_name, opt_kwargs in configs:
    print(f"\n=== {name} ===")
    ptorch_config.use_projections = use_proj
    ...
    for step in range(500):
        ...
        if step % 100 == 0 or step == 499:
            ...
            for n, p in model.named_parameters():
                if 'weight' in n:
                    w_norms[n.split('.')[0]] = f"{p.norm():.3f}"

            print(f"  step={step:4d}  loss={ce_loss:.4f}  acc={train_acc:.4f}  val={val:.4f}  feat_std={feat_std:.4f}  weights={w_norms}")
