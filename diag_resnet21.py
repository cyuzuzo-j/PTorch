"""Diagnostic 21: Reproduce benchmark conditions (bs=512) and isolate norm_preserve."""
import sys, os
...
configs = [
    ("FrozenA, Muon0.5, norm_pres=T, bs512", True, 0.5, True, 512),
    ("FrozenA, Muon0.5, norm_pres=F, bs512", True, 0.5, False, 512),
    ("FrozenA, Muon0.5, norm_pres=F, bs256", True, 0.5, False, 256),
    ("FrozenA=F, Muon0.5, norm_pres=F, bs512", False, 0.5, False, 512),
]

for name, frozen_a, act_lr, norm_pres, bs in configs:
    print(f"\n=== {name} ===")
    ...
    ptorch_config.config.update("muon_activations_norm_preserve", norm_pres)

    torch.manual_seed(42)
    model = ResNet8_PTorch(classes=10, in_channels=3).to(device)
    ds = InfiniteCifarDataModule(batch_size=bs, data_dir="./dataset", seed=42)
    ...
    for step in range(5000):
        ...
        if step % 1000 == 0 or step == 4999:
            ...
            print(f"  step={step:5d}  loss={ce_loss:.4f}  acc={train_acc:.4f}  val={val:.4f}  best={best_val:.4f}  feat_std={feat_std:.4f}")
