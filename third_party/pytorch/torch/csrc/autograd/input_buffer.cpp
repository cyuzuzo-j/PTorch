...
auto InputBuffer::variables(InputBuffer&& g) -> std::vector<Variable> {
  ...
  bool average_grads = false;
  if (const char* env_p = std::getenv("PTORCH_AVERAGE_GRADIENTS")) {
    if (std::strcmp(env_p, "1") == 0 || std::strcmp(env_p, "True") == 0) {
      average_grads = true;
    }
  }
  ...
  if (average_grads) {
    for (size_t i = 0; i < result.size(); i++) {
      if (result[i].defined() && g.accum_counts[i] > 1) {
        result[i] = result[i] / g.accum_counts[i];
      }
    }
  }
  return result;
