"""Diagnostic 20: Fix feature shrinkage - compare activation target strategies."""
import sys, os
...
configs = [
    ("Muon lr=0.5 (current best)", True, 0.5, False),
    ("No Muon (raw A_proj)", False, 1.0, False),
    ("Muon lr=0.5 + norm preserve", True, 0.5, True),
    ("Muon lr=1.0 + norm preserve", True, 1.0, True),
]

for name, muon_on, act_lr, norm_pres in configs:
    print(f"\n=== {name} ===")
    ptorch_config.config.update("frozen_a_weights", True)
    ptorch_config.config.update("muon_weights", False)
    ptorch_config.config.update("muon_activations", muon_on)
